    ...
    function __init() {
        ...
        $.ajax({ //获取验证码
            ...
            success: function (result) {
                ...
                var errcode = result['data']['errcode'];
                if (errcode !== 0) {
                    document.getElementById(__codediv).innerHTML =
                        "<span style='color:red'>\u9a8c\u8bc1\u7801\u83b7\u53d6\u5931\u8d25\u002c"
                        + result['data']['errmsg'] + "</span>";
                }
				
                var yvalue = result['data']['y'], small = result['data']['small'], array = result['data']['array'], normal = result['data']['normal'];
                __imgx = result['data']['imgx'];
                __imgy = result['data']['imgy'];
